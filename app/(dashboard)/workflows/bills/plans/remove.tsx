...
function RemovePlan({ plan, queryKey }: { plan: Plan; queryKey: QueryKey }) {
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction
            ...
            onClick={async () => {
              try {
                setLoading(true);
                const response = await axios.delete(`/api/plans/${plan.id}`);
                toast({ title: "Success", description: "Remove successfully" });
                updateData({ queryKey, id: plan.id, remove: true });
              } catch (error: any) {
                toast({
                  title: "Error",
                  variant: "destructive",
